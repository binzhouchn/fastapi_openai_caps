...
async def chat_completions(body: ChatBody, request: Request, background_tasks: BackgroundTasks):
    ...
    if body.stream:
        def eval_llm():
            ...
            for _response in f'#[test] question: {question}@#@#|history: {history}#':
                response += _response
                if first:
                    first = False
        ...
        # response = ''.join(do_chat(question, history=history, **chat_kwargs))#修改
        response = f'####[test] question: {question}@#@#|history: {history}##END'
